...
void OnroadWindow::mousePressEvent(QMouseEvent* e) {
  ...
  if (longitudinal_plan.getSpeedLimit() > 0.0 && speed_limit_touch_rect.contains(e->x(), e->y())) {
    ...
    QUIState::ui_state.scene.dev_ui_enabled = !QUIState::ui_state.scene.dev_ui_enabled;
    if (QUIState::ui_state.scene.dev_ui_enabled) {
      Params().putBool("DevUI", true);
    } else {
      Params().putBool("DevUI", false);
    }
    propagate_event = false;
  }
  else if (map != nullptr) {
...
void OnroadHud::paintEvent(QPaintEvent *event) {
  ...
  /*p.setPen(QPen(QColor(0xff, 0xff, 0xff, 100), 10));
  p.setBrush(QColor(0, 0, 0, 100));
  p.drawRoundedRect(rc2, 20, 20);
  p.setPen(Qt::NoPen);*/

  if (showDebugUI && devUiEnabled) {
    drawLeftDevUi(p, rect().right() - 184 - bdr_s * 2, bdr_s * 2 + rc2.height());
  }
}
...
int OnroadHud::drawDevUiElement(QPainter &p, int x, int y, const char* value, const char* label, const char* units, QColor &color) {
...
void OnroadHud::drawLeftDevUi(QPainter &p, int x, int y) {
  ...
  if (true) {
    ...
    rh += drawDevUiElement(p, x, ry, val_str, "REL DIST", units_str, valueColor);
    ry = y + rh;
  }
  ...
  if (true) {
    ...
    rh += drawDevUiElement(p, x, ry, val_str, "REL SPEED", speedUnit.toStdString().c_str(), valueColor);
    ry = y + rh;
  }
  ...
  if (true) {
    ...
    rh += drawDevUiElement(p, x, ry, val_str, "REAL STEER", "", valueColor);
    ry = y + rh;
  }
  ...
  if (true) {
    ...
    if (QUIState::ui_state.scene.controls_state.getMadsEnabled()) {
      // Red if large steering angle
      // Orange if moderate steering angle
      if (std::fabs(angleSteers) > 12) {
        valueColor = QColor(255, 0, 0, 255);
    ...
    rh += drawDevUiElement(p, x, ry, val_str, "DESIR STEER", "", valueColor);
    ry = y + rh;
  }
  ...
  if (true) {
    ...
    rh += drawDevUiElement(p, x, ry, val_str, "ENG RPM", "", valueColor);
    ry = y + rh;
  }
