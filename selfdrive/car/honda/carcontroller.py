...
class CarController:
  ...
  def update(self, CC, CS):
    ...
    beep = slc_active and CS.out.cruiseState.enabled and not ((cur_time - self.beep_timer) > 0.5)

    if self.speed_limit_prev == self.speed_limit:
      self.beep_timer = cur_time

    speed_limit_changed = self.speed_limit_prev != self.speed_limit and not ((cur_time - self.beep_timer) > 1)
    ...
    if self.frame % 10 == 0:
      ...
      can_sends.extend(hondacan.create_ui_commands(self.packer, self.CP, pcm_speed, hud, CS.is_metric, idx, CS.stock_hud, CS.gap_adjust_cruise_tr, beep, speed_limit_changed))
  ...
  def get_cruise_buttons_status(self, CS):
    if not CS.out.cruiseState.enabled or CS.cruise_buttons[-1] != 0:
      self.timer = 40
    elif self.timer:
      self.timer -= 1
    else:
